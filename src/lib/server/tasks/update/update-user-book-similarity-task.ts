...
async function getValidMedia(
  clientId: number,
  minCommonReviews: number,
): Promise<Review[]> {
...
export default class UpdateUserBookSimilarityTask extends Task {
  protected async _Run(): Promise<void> {
    ...
    for (let _i in allClients) {
      ...
      let mediaSimilarities = Object.entries(reviewsForMedia).map(
        ([mediaId, reviews]) => {
          let weightedSum = reviews.reduce((total, review) => {
            ...
            let weightedScore =
              usersById[review.userId].similarity * (score / 10);
